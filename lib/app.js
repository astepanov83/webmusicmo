...
export function createApp({ stationsFile, fetchImpl = fetch, cacheMs = 5000, preferHttps = true, checkMs = 60_000, log = console.warn } = {}) {
  if (!stationsFile) throw new Error('stationsFile is required');
  const fix = preferHttps ? toHttps : (u) => u;
  const store = createStationStore({ file: stationsFile, checkMs, log });
  // Read the station file once at start so a missing or broken file is logged now,
  // not discovered later as a blank spot in someone's browser.
  store.load().catch((err) => log(`stations: could not read ${stationsFile} at startup: ${err.message}`));
  ...
  // One stream per playlist: the first entry of each PLS, probed. The first playlist that
  // could be read is the primary one. The player stays on the endpoint it is given and just
  // retries it, so the remaining entries of a playlist are not passed on.
  ...
  async function apiStreams(station) {
    const { streams: resolved, plsUrl } = await resolvePlaylists(station);
  ...
  return function handler(req, res) {
    ...
    // decodeURIComponent('/%00') succeeds and gives a string with a null byte. fs.stat throws
    // synchronously on that, uncaught, which kills the whole process. Reject it here instead.
    if (decoded.includes('\0')) return badRequest(res);
