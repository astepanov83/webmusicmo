...
export async function refreshStations({ file, localFile, somafmUrl, fetchImpl = fetch, log = () => {} } = {}) {
  ...
  try {
    ...
    const channels = parseChannels(JSON.parse(await res.text()));
    // Validate each channel on its own: one bad row from SomaFM must not take down
    // every other station along with it.
    somafm = [];
    for (const c of channels) {
      try {
        somafm.push(validateStation(c));
      } catch (err) {
        log(`refresh: skipping SomaFM channel ${c && c.id}: ${err.message}`);
      }
    }
