...
// server_url comes from a third-party Icecast status page, not from our own validated station
// data, and it lands straight in the footer link's href when a station has no site of its own.
// Require http or https, same as the station validator does, and fall back to empty otherwise
// so the page's existing fallback chain takes over instead of a scheme like "javascript:".
function httpUrlOnly(value) {
...
export function parseStatus(json) {
  ...
  return {
    ...
    stationUrl: httpUrlOnly(pick('server_url')),
